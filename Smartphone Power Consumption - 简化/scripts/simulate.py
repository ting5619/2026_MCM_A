...
from models.types import UsageInputs
...
def _wrap_usage_with_ambient(usage_fn: Callable[[float], UsageInputs], t_amb_c: float) -> Callable[[float], UsageInputs]:
    """把环境温度写回 UsageInputs（若字段存在），便于场景调参/记录。"""

    def f(t_s: float) -> UsageInputs:
        u = usage_fn(t_s)
        return replace(u, ambient_c=float(t_amb_c)) if hasattr(u, "ambient_c") else u
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "运行示例：python -m scripts.simulate --scenario piecewise_day --compare"
        )
    )
